...
export function LoginPage({ authError }: { authError?: string | null }) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) {
      // Supabase reports an unconfigured provider as "Unsupported provider",
      // which reads like the app asked for something that doesn't exist. It's a
      // setting nobody has turned on yet, and the person seeing this screen is
      // usually the one who can turn it on.
      setError(
        /provider is not enabled|unsupported provider/i.test(error.message)
          ? 'Google sign-in is not enabled for this project. Use an email address and password below, or enable the Google provider in Supabase under Authentication → Sign In / Providers.'
          : error.message
      )
      setLoading(false)
    }
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 px-4 py-10">
      ...
      <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-xl">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-gray-900">Tithing Declaration</h1>
          <p className="mt-2 text-gray-600">Scheduling for ward leadership</p>
          <p className="mt-4 text-sm text-gray-500">
            Members don't need an account — signing in is for the executive
        ...
        <p className="mt-6 text-center text-xs text-gray-500">
          A new account can't see any schedule until an admin grants it access.
        </p>
      </div>
    </div>
  )
